...
import type { QrCodeRow, QrErrorLevel, QrScanRow, QrVisibility } from '@allebrum/shared';
...
type Props = {
  ...
  defaultVisibility?: QrVisibility;
...
export function QrCodeManager({
  ...
  createHeading = 'New code',
  emptyTitle = 'No codes yet',
  emptyHint = 'Create one above to start tracking scans.',
  onOpenClient,
  onOpenProject,
}: Props) {
  const { me } = useAuth();
  ...
  const codes = useQrCodes({ clientId, projectId });
  const create = useCreateQrCode();
  const toast = useToast();

  const [label, setLabel] = useState('');
  const [targetUrl, setTargetUrl] = useState('');
  const [projectLinkId, setProjectLinkId] = useState(projectId ?? '');
  ...
  const onCreate = async () => {
    ...
    const selectedProjectId = projectId ?? (allowProjectLinkPicker ? (projectLinkId || null) : null);
    try {
      const row = await create.mutateAsync({
        label: label.trim(),
        targetUrl: url,
        visibility: defaultVisibility,
        clientId: selectedProjectId ? null : clientId,
        projectId: selectedProjectId,
      });
      setLabel('');
      setTargetUrl('');
      if (!projectId) setProjectLinkId('');
      setOpenId(row.id);
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-2xl border border-gray-200 bg-white p-5">
        ...
        {allowProjectLinkPicker && clientId && !projectId && (
          <div className="mt-3 max-w-sm">
            <Field label="Link to project (optional)">
              <Select value={projectLinkId} onChange={(e) => setProjectLinkId(e.target.value)}>
                <option value="">Client-level QR (not project-linked)</option>
          ...
          <ul className="rounded-2xl border border-gray-200 bg-white divide-y divide-gray-100">
            {codes.data.map((c) => (
              <CodeRow
                key={c.id}
                code={c}
                clientId={c.clientId ?? (c.projectId ? (projectsById.get(c.projectId)?.clientId ?? null) : null)}
                clientName={
                  c.clientId
                    ? (clientsById.get(c.clientId) ?? null)
                    : c.projectId
                      ? (clientsById.get(projectsById.get(c.projectId)?.clientId ?? '') ?? null)
                      : null
                }
                projectId={c.projectId}
                projectName={c.projectId ? (projectsById.get(c.projectId)?.name ?? null) : null}
        ...
        <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[10px] text-gray-500">
          {clientName && (
            <button
              type="button"
              onClick={() => {
                if (clientId && onOpenClient) onOpenClient(clientId);
              }}
              ...
              title={clientId && onOpenClient ? `Open client ${clientName}` : undefined}
              disabled={!clientId || !onOpenClient}
            >
              {`Client · ${clientName}`}
            </button>
          )}
          {projectName && (
            <button
              ...
              title={projectId && onOpenProject ? `Open project ${projectName}` : undefined}
              disabled={!projectId || !onOpenProject}
            >
              {`Project · ${projectName}`}
            </button>
          )}
          <span className="inline-flex items-center rounded-full px-2 py-0.5 bg-emerald-50 text-emerald-700 font-semibold uppercase tracking-wider">
            {code.uniqueVisitors} visitors
          </span>
          <span className="inline-flex items-center rounded-full px-2 py-0.5 bg-gray-100 text-gray-600 font-semibold uppercase tracking-wider">
